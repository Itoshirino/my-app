...
export default function HomePage() {
  ...
  const getPrices = () => {
    if (!selectedTariff) return { basePriceStr: "", totalPriceStr: "", count: 0 }
    ...
    const totalPriceStr = totalPriceNum.toLocaleString("ru-RU").replace(/,/g, " ")
    return {
      ...
      setInsuredList(prev => {
        ...
        if (prev.length < count) {
          const added = Array.from({ length: count - prev.length }, (_, i) => ({
            id: Date.now() + i, passport: "", birthDate: "", phone: "", fullName: ""
          }))
          return [...prev, ...added]
        }
  ...
  const handlePayerNameClick = () => {
    if (!payerPassport || !payerBirthDate) {
      alert("Пожалуйста, сначала заполните поля «Серия и номер паспорта» и «Дата рождения»")
      return
    }
    ...
    if (searchKey in mockDatabase) {
      ...
      setErrors(prev => { const { payerFullName, ...rest } = prev; return rest })
    } else {
      alert("Гражданин с такими данными паспорта и даты рождения не найден в базе данных.")
    }
  }

  const handleInsuredNameClick = (id: number, passport: string, birthDate: string) => {
    if (!passport || !birthDate) {
      alert("Пожалуйста, сначала заполните поля «Серия и номер паспорта» и «Дата рождения» для этого лица")
      return
    }
    ...
    if (searchKey in mockDatabase) {
      ...
      setErrors(prev => { const { [`insured_${id}_fullName`]: _, ...rest } = prev; return rest })
    } else {
      alert("Гражданин с такими данными паспорта и даты рождения не найден в базе данных.")
    }
  }
  ...
  const validateTab1 = () => {
    const newErrors: { [key: string]: string } = {}
    if (!payerPassport) newErrors.payerPassport = "Заполните серию и номер паспорта плательщика"
    if (!payerBirthDate) newErrors.payerBirthDate = "Укажите дату рождения плательщика"
    if (!payerPhone || payerPhone.length < 12) newErrors.payerPhone = "Введите корректный номер телефона"
    if (!payerFullName) newErrors.payerFullName = "Нажмите на иконку поиска, чтобы заполнить ФИО из базы данных"
    if (!isAgreed) newErrors.isAgreed = "Необходимо подтвердить согласие с условиями оферты"

    insuredList.forEach((insured, index) => {
      if (!insured.passport) newErrors[`insured_${insured.id}_passport`] = `Заполните паспорт лица #${index + 1}`
      if (!insured.birthDate) newErrors[`insured_${insured.id}_birthDate`] = `Укажите дату рождения лица #${index + 1}`
      if (!insured.phone || insured.phone.length < 12) newErrors[`insured_${insured.id}_phone`] = `Введите номер телефона лица #${index + 1}`
      if (!insured.fullName) newErrors[`insured_${insured.id}_fullName`] = `Заполните ФИО лица #${index + 1} через поиск`
    })
  ...
  const handlePhoneInput = (val: string, setter: (v: string) => void, fieldKey?: string) => {
    ...
    if (fieldKey && finalVal.length === 12) {
      setErrors(prev => { const { [fieldKey]: _, ...rest } = prev; return rest })
    }
  }
  ...
  const updateInsuredField = (id: number, field: string, value: string) => {
    setInsuredList(insuredList.map(item => {
      if (item.id === id) {
        const updated = { ...item, [field]: field === 'passport' ? value.toUpperCase() : value }
        return updated
      }
      return item
    }))
    setErrors(prev => { const { [`insured_${id}_${field}`]: _, ...rest } = prev; return rest })
  }
    ...
    <div className="min-h-screen bg-gray-50 text-gray-800">
      <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col md:flex-row gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 bg-gray-50 border rounded-lg focus:outline-none focus:ring-2 focus:bg-white transition-all ${
                      errors.insuredCount ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                    }`}
                  />
                  {errors.insuredCount && (
                    <p className="text-red-500 text-xs mt-1.5 font-medium flex items-center gap-1"> {errors.insuredCount}</p>
                  )}
                </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 bg-gray-50 border rounded-lg focus:outline-none focus:ring-2 focus:bg-white cursor-pointer transition-all ${
                        errors.startDate ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                      }`}
                    />
                    {errors.startDate && (
                      <p className="text-red-500 text-xs mt-1.5 font-medium flex items-center gap-1"> {errors.startDate}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-sm text-gray-700 mb-2 font-medium">
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className={`w-full px-4 py-3.5 bg-gray-50 border rounded-xl flex justify-between items-center text-left focus:outline-none focus:ring-2 focus:bg-white transition-all ${
                      errors.selectedTariff ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                    }`}
                  >
                    {selectedTariff ? (
                      <span className="text-sm text-gray-700">
                        Стоимость полиса: <span className="font-bold text-gray-900">{tariffs[selectedTariff].price}</span>, Сумма покрытия: <span className="font-bold text-gray-900">{tariffs[selectedTariff].coverage}</span>
                      </span>
                    ) : (
                      <span className="text-gray-400 text-sm">Выберите тариф</span>
                    )}
                    <span className={`text-gray-400 transition-transform ${isTariffOpen ? "rotate-180" : ""}`}>
                      ▼
                    </span>
                  </button>

                  {errors.selectedTariff && (
                    <p className="text-red-500 text-xs mt-1.5 font-medium flex items-center gap-1"> {errors.selectedTariff}</p>
                  )}
                      ...
                      {Object.entries(tariffs).map(([key, val]) => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            setErrors(prev => { const { selectedTariff, ...rest } = prev; return rest })
                          }}
                        ...
                        >
                          <div className="flex justify-between">
                            ...
                            <span className="font-bold text-gray-900">{val.price}</span>
                          </div>
                          <div className="flex justify-between text-xs text-gray-500">
                            <span>Сумма покрытия:</span>
                            <span className="font-semibold text-gray-800">{val.coverage}</span>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {activeTab === 1 && (
              <div className="space-y-8">
                ...
                <div className="border-b border-gray-100 pb-6">
                  <h3 className="text-lg font-bold text-emerald-800 mb-4">Плательщик</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm text-gray-600 mb-1 font-medium">
                        <span className="text-red-500 mr-1">*</span> Серия и номер паспорта:
                      </label>
                      <input
                        type="text"
                        ...
                        onChange={(e) => {
                          ...
                          setErrors(prev => { const { payerPassport, ...rest } = prev; return rest })
                        }}
                        className={`w-full px-4 py-2.5 bg-gray-50 border rounded-lg focus:outline-none focus:ring-2 focus:bg-white transition-all ${
                          errors.payerPassport ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                        }`}
                      />
                      {errors.payerPassport && (
                        <p className="text-red-500 text-xs mt-1 font-medium"> {errors.payerPassport}</p>
                      )}
                    </div>
                    <div>
                      <label className="block text-sm text-gray-600 mb-1 font-medium">
                        <span className="text-red-500 mr-1">*</span> Дата рождения:
                      </label>
                      <input
                        type="date"
                        ...
                        onChange={(e) => {
                          ...
                          setErrors(prev => { const { payerBirthDate, ...rest } = prev; return rest })
                        }}
                        className={`w-full px-4 py-2.5 bg-gray-50 border rounded-lg focus:outline-none focus:ring-2 focus:bg-white transition-all ${
                          errors.payerBirthDate ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                        }`}
                      />
                      {errors.payerBirthDate && (
                        <p className="text-red-500 text-xs mt-1 font-medium"> {errors.payerBirthDate}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                      <label className="block text-sm text-gray-600 mb-1 font-medium">
                        <span className="text-red-500 mr-1">*</span> Номер телефона:
                      </label>
                      <input
                        type="tel"
                        placeholder="+998 00 123 45 67"
                        value={formatPhoneDisplay(payerPhone)}
                        onChange={(e) => handlePhoneInput(e.target.value, setPayerPhone, "payerPhone")}
                        className={`w-full px-4 py-2.5 bg-gray-50 border rounded-lg focus:outline-none focus:ring-2 focus:bg-white transition-all ${
                          errors.payerPhone ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                        }`}
                      />
                      {errors.payerPhone && (
                        <p className="text-red-500 text-xs mt-1 font-medium"> {errors.payerPhone}</p>
                      )}
                    </div>
                    
                    <div>
                      <label className="block text-sm text-gray-600 mb-1 font-medium">
                        <span className="text-red-500 mr-1">*</span> Имя и фамилия:
                      </label>
                      <div className="relative">
                        <input
                          type="text"
                          ...
                          className={`w-full pl-4 pr-10 py-2.5 bg-gray-50 border rounded-lg text-sm cursor-pointer text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:bg-white transition-all ${
                            errors.payerFullName ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                          }`}
                        />
                        <button 
                          type="button"
                      ...
                      {errors.payerFullName && (
                        <p className="text-red-500 text-xs mt-1 font-medium"> {errors.payerFullName}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <h3 className="text-lg font-bold text-emerald-800 mb-4">Застрахованное лицо(а)</h3>
                  
                  {insuredList.map((insured, index) => (
                    <div key={insured.id} className="bg-gray-50/50 p-4 rounded-xl border border-gray-150 mb-4 space-y-4 relative">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                        <div>
                          <label className="block text-sm text-gray-600 mb-1 font-medium">
                            <span className="text-red-500 mr-1">*</span> Серия и номер паспорта лица #{index + 1}:
                          </label>
                          <input
                            type="text"
                            placeholder="AA7654321"
                            value={insured.passport}
                            onChange={(e) => updateInsuredField(insured.id, 'passport', e.target.value)}
                            className={`w-full px-4 py-2.5 bg-white border rounded-lg focus:outline-none focus:ring-2 transition-all ${
                              errors[`insured_${insured.id}_passport`] ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                            }`}
                          />
                          {errors[`insured_${insured.id}_passport`] && (
                            <p className="text-red-500 text-xs mt-1 font-medium"> {errors[`insured_${insured.id}_passport`]}</p>
                          )}
                        </div>
                        <div>
                          <label className="block text-sm text-gray-600 mb-1 font-medium">
                            <span className="text-red-500 mr-1">*</span> Дата рождения лица #{index + 1}:
                          </label>
                          <input
                            type="date"
                            value={insured.birthDate}
                            onChange={(e) => updateInsuredField(insured.id, 'birthDate', e.target.value)}
                            className={`w-full px-4 py-2.5 bg-white border rounded-lg focus:outline-none focus:ring-2 transition-all ${
                              errors[`insured_${insured.id}_birthDate`] ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                            }`}
                          />
                          {errors[`insured_${insured.id}_birthDate`] && (
                            <p className="text-red-500 text-xs mt-1 font-medium"> {errors[`insured_${insured.id}_birthDate`]}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm text-gray-600 mb-1 font-medium">
                            <span className="text-red-500 mr-1">*</span> Номер телефона лица #{index + 1}:
                          </label>
                          <input
                            type="text"
                            placeholder="+998 00 123 45 67"
                            ...
                            onChange={(e) => {
                              ...
                              updateInsuredField(insured.id, 'phone', value.slice(0, 12))
                            }}
                            className={`w-full px-4 py-2.5 bg-white border rounded-lg focus:outline-none focus:ring-2 transition-all ${
                              errors[`insured_${insured.id}_phone`] ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                            }`}
                          />
                          {errors[`insured_${insured.id}_phone`] && (
                            <p className="text-red-500 text-xs mt-1 font-medium"> {errors[`insured_${insured.id}_phone`]}</p>
                          )}
                        </div>
                        
                        <div>
                          <label className="block text-sm text-gray-600 mb-1 font-medium">
                            <span className="text-red-500 mr-1">*</span> Имя и фамилия застрахованного:
                          </label>
                          <div className="relative">
                            <input
                              type="text"
                              ...
                              onClick={() => handleInsuredNameClick(insured.id, insured.passport, insured.birthDate)}
                              placeholder="Имя и фамилия"
                              className={`w-full pl-4 pr-10 py-2.5 bg-white border rounded-lg text-sm cursor-pointer text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 transition-all ${
                                errors[`insured_${insured.id}_fullName`] ? "border-red-400 focus:ring-red-400 bg-red-50/30" : "border-gray-200 focus:ring-emerald-600"
                              }`}
                            />
                            <button 
                              type="button"
                          ...
                          {errors[`insured_${insured.id}_fullName`] && (
                            <p className="text-red-500 text-xs mt-1 font-medium"> {errors[`insured_${insured.id}_fullName`]}</p>
                          )}
                        </div>
                      </div>
                ...
                <div className="pt-2">
                  <div className="flex items-start gap-3">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.checked) setErrors(prev => { const { isAgreed, ...rest } = prev; return rest })
                      }}
                      className="w-4 h-4 mt-1 accent-emerald-700 rounded focus:ring-emerald-600"
                    />
                    <label htmlFor="oferta" className="text-sm text-gray-600 select-none">
                  ...
                  {errors.isAgreed && (
                    <p className="text-red-500 text-xs mt-1 font-medium ml-7"> {errors.isAgreed}</p>
                  )}
                </div>
                    ...
                    >
                      <div className="h-10 flex items-center justify-center">
                        <svg height="32" viewBox="0 0 120 32" xmlns="http://www.w3.org/2000/svg">
                          <text x="50%" y="24" textAnchor="middle" fontFamily="Arial, Helvetica, sans-serif" fontWeight="900" fontSize="28" fill="#00A5FF" letterSpacing="-1">CLICK</text>
                        </svg>
                      </div>
        ...
        <div className="w-full md:w-72 bg-white rounded-2xl border border-gray-200 p-6 h-fit sticky top-8 shadow-sm">
          <h3 className="text-base font-bold text-emerald-800 mb-4 border-b pb-2">Результаты расчета</h3>
          <div className="space-y-4 text-sm">
              ...
              return (
                <>
                  <div>
                    ...
                    <p className="font-bold text-emerald-900">{tariffs[selectedTariff].coverage} UZS</p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-xs">Цена самого тарифа:</p>
                    <p className="font-semibold text-gray-700">{basePriceStr} UZS</p>
                  </div>
                  <div className="pt-2 border-t border-dashed border-gray-200 bg-emerald-50/40 p-2 rounded-lg">
                    <p className="text-gray-500 text-xs font-medium">Итоговая цена:</p>
                    <p className="font-black text-emerald-800 text-base">{totalPriceStr} UZS</p>
                    {count > 1 && (
            ...
            {payerFullName && (
              <div className="pt-2 border-t border-gray-100">
                ...
                <p className="font-semibold text-gray-900 truncate">{payerFullName}</p>
              </div>
            )}
            {payerPhone && (
              <div>
                <p className="text-gray-400 text-xs">Телефон:</p>
                <p className="font-semibold text-gray-900">{formatPhoneDisplay(payerPhone)}</p>
              </div>
            )}
            ...
            {!startDate && !selectedTariff && (
              <p className="text-gray-400 text-xs italic">Заполните данные для начала расчета стоимости.</p>
            )}
          </div>
        </div>
